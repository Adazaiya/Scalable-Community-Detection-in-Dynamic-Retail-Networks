...
PRODUCT_CATALOG = {
    ...
    'pantry': ['Pasta', 'Rice', 'Beans', 'Tomato Sauce', 'Soup', 'Cereal',
    ...
    'household': ['Dish Soap', 'Laundry Detergent', 'All-Purpose Cleaner', 'Sponges',
...
# SHOPPING PATTERNS
SHOPPING_PATTERNS = {
    'breakfast': {'primary': ['dairy', 'breakfast'], 'secondary': ['beverages', 'produce', 'bakery'], 'weight': 0.15},
    'meal_prep': {'primary': ['meat', 'produce'], 'secondary': ['pantry', 'dairy', 'frozen'], 'weight': 0.20},
    'healthy': {'primary': ['produce'], 'secondary': ['dairy', 'beverages', 'breakfast'], 'weight': 0.15},
    'convenience': {'primary': ['frozen', 'snacks'], 'secondary': ['beverages', 'bakery'], 'weight': 0.10},
    'weekly': {'primary': ['dairy', 'produce', 'meat'], 'secondary': ['pantry', 'bakery', 'beverages', 'snacks', 'frozen'], 'weight': 0.20},
    'essentials': {'primary': ['personal_care', 'household'], 'secondary': ['dairy', 'produce'], 'weight': 0.10},
    'random': {'primary': list(PRODUCT_CATALOG.keys()), 'secondary': [], 'weight': 0.10}
}
...
for order_id in range(1, NUM_ORDERS + 1):
    ...
    order_date = START_DATE + timedelta(days=random.randint(0, 180))  # increased range
...
for order in orders:
    ...
    for _ in range(num_items):
        r = random.random()
        if r < 0.45:
            categories = pattern_config['primary'] if pattern_config['primary'] else list(PRODUCT_CATALOG.keys())
        elif r < 0.9:
            categories = pattern_config['secondary'] if pattern_config['secondary'] else list(PRODUCT_CATALOG.keys())
        else:
            categories = list(PRODUCT_CATALOG.keys())  # random selection
